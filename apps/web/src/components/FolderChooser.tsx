...
interface Props {
  ...
  /** true 면 하나만 고른다 (설정 카드). false 면 여러 개 (설정 화면). */
  single?: boolean;
  /** 이미 고른 폴더는 목록에서 뺀다 (설정 화면의 '폴더 추가') */
      ...
      {rows.map((f, i) => {
        ...
        return (
          <button
            ...
            className={`flex h-11 items-center gap-2.5 px-3 text-left ${i > 0 || suggest.isPending ? 'border-t border-line-soft' : ''} ${on ? 'bg-accent-soft' : ''}`}
          >
            <span
              className={`flex h-4 w-4 flex-none items-center justify-center rounded-pill border ${on ? 'border-accent bg-accent text-white' : 'border-line'}`}
            >
              {on && <CheckIcon />}
            </span>
            <span className="flex min-w-0 flex-1 flex-col gap-px">
              <span className="text-13 font-medium">{f.label}</span>
              <span className="truncate text-11 text-text-2">{f.path}</span>
            </span>
            <span className={`flex-none text-12 ${f.videoCount === 0 ? 'text-text-2' : 'text-text-3'}`}>{copy.folders.count(f.videoCount)}</span>
          </button>
        );
      })}
      <button
        ...
        className={`flex h-11 items-center px-3 text-left text-13 font-medium text-accent ${rows.length > 0 || suggest.isPending ? 'border-t border-line-soft' : ''}`}
      >
...
function Row({ children, muted }: { children: React.ReactNode; muted?: boolean }) {
  return <div className={`flex min-h-11 items-center px-3 py-2 text-12 leading-snug ${muted ? 'text-text-2' : ''}`}>{children}</div>;
}
