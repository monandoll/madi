...
| B-2 구도 | **통과 (대용 원본)** | 16:9 롱폼을 촬영 원본 대용으로 써서 판정했다. 아래 참고 |
...
**1단계는 `G1 · G2 의 정의`가 정해진 뒤에 시작한다.**
크리에이터 촬영 원본은 기다리지 않는다 — 유튜브의 다른 운동·재활 롱폼을 대용으로 쓴다.

### B-2 를 판정하는 법 (촬영 원본 대용)

크리에이터 공개본은 **이미 9:16 으로 크롭된 완성본**이라 리프레이밍을 검증할 수 없다.
대신 유튜브의 다른 운동·재활 롱폼을 `spike/source/raw/` 에 둔다. **커밋하지 않는다.**

조건:
- 16:9, 1080p 이상
- 서 있는 자세 + 바닥·침대에 누운 자세가 모두 있을 것
- 2인(치료사 + 회원) 1편 이상, 고정 카메라 1편 이상, 가능하면 세로 원본 1편
- 3~5편

판정:

1. **숫자** — 뽑은 9:16 에서 인물 높이 비율이
   `reframe.targetSubjectHeightRatio` **±0.08** 안에 드는가.
   인물 높이는 `VNGeneratePersonSegmentationRequest` 마스크 상자로 잰다
   (`madi-spike detect --box 1`). 관절 상자로 재지 않는다 — 머리 꼭대기가 빠진다.
2. **눈** — `reference/` 프레임과 나란히 놓고 **같은 채널 구도로 보이는가.**
   프레임을 커밋에 첨부한다.

0단계에서는 리프레임 좌표를 **손으로 적는다.** 자동 리프레이밍은 1단계다.
