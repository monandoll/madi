...
엔진은 `127.0.0.1` 에만 붙는다. 같은 와이파이라도 폰이 직접 못 들어오고, **설정 → 밖에서 접속하기 → 켜기** 를 해야 한다 (계정·토큰 없음, `docs/remote.md`). PC 안내 카드가 QR 과 잠금 숫자를 그대로 보여 준다. 터널 뒤에서도 주소가 맞도록 tus 는 상대 Location 을 준다.

밖에서 온 올리기도 짝지은 기기만 된다 — `/api/*` 는 표가 없으면 401 `needs_pair` 다.
