...
| G4 | 자막 크기 | 글자 높이 >= 프레임 높이 4.5% |
| G5 | 자막 분절 | 한 덩어리 <= 12자, 2줄 이내 |
...
- **Remotion**: 회사 규모에 따라 상업 라이선스가 필요하다. 0단계 시작 전에 확인한다. 조건이 맞지 않으면 WebCodecs + canvas 자체 컴포지터로 전환하고, 그 비용을 일정에 반영한다.
