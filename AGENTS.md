...
struct Composition: Codable {
    ...
    var captionSlot: CaptionSlot    // 필수. 영상 단위로 하나 (아래 ★)
...
struct Scene: Codable {
    ...
    var captionSlot: CaptionSlot?   // 덮어쓰기. 웬만하면 비워 둔다
}

/// 자막 블록을 화면 어디에 놓을지. **이름은 내용어다. 좌표를 암시하지 않는다.**
enum CaptionSlot: String, Codable {
    case upperBody   // 앉거나 서서 말하는 상반신
    case fullBody    // 서 있는 전신
...
`captionSlot` 은 `Composition` 에 있고 `Caption` 에는 없다.

- 공개 숏폼 10편에서 **편 안에서는 12프레임 전부 같은 위치**였다
  (`8DF9jrxQM4U` 는 12프레임이 전부 0.2354). 크리에이터는 한 영상 안에서 자막을 옮기지 않는다
- 편 **사이**에서는 0.235 ~ 0.483 으로 갈렸다. 프레임 높이의 25% 다
- 그래서 값은 하나로 못 박고(`§9` 슬롯 3개), 고르는 단위는 영상이다
- `Scene.captionSlot` 은 덮어쓰기용이다. **웬만하면 비워 둔다** —
  한 영상 안에서 자막이 오르내리면 그 순간 아마추어 편집처럼 보인다
- `captionSlot` 에 **기본값을 두지 않는다.** 안 적으면 조용히 아래에 붙는 게 아니라 파싱이 실패한다

근거: `docs/findings/2026-09-25-caption-position-10.md`.

⚠ 자막이 피사체를 피해 움직인다는 가설은 10편으로 검증해 **기각됐다.**
크리에이터는 자막을 피사체 **위에** 얹는다. 기하로 자동 결정할 수 없어서 **고르게** 한다
...
| # | 항목 | 기준 |
|---|---|---|
| G1 | 피사체 크기 | **사람 높이** >= 프레임 높이 55% 인 구간이 전체의 80% 이상 |
| G2 | 피사체 잘림 | **리프레이밍이 새로 자른** 구간 5% 이하 (원본에서 이미 잘린 건 세지 않는다) |
...
| G8 | 훅 | 첫 장면 `role == .hook` 이고 첫 자막이 **0.5초 안에** 시작 (공개본 10/10 이 0.0초 — `docs/findings/2026-09-26-hook-titlecard-10.md`) |
...
근거: `docs/findings/2026-09-25-g1-g2-definition-proposal.md` (공개본 10편 실측).
...
- **관절로 재지 않는다.** 관절 상자는 머리 꼭대기가 빠지고(최상단 관절이 눈·귀·코)
  클로즈업에서 아예 안 잡힌다. 60프레임에서 관절 37 · 분할 **60**
- **G1 · G2 는 렌더 결과가 아니라 "원본 프레임 마스크 + 리프레임 좌표" 로 계산한다.**
  원본에는 우리가 얹을 오버레이 카드가 없고, 카드 위치는 `Composition` 이 안다.
  카드 속 인물을 피사체로 오인하는 사고가 **구조적으로** 사라진다
- 마스크가 비면 직전 값을 유지하고, 장면 전체가 비면 중앙 크롭 + **측정 불가**로 기록한다.
  **측정 불가가 길이의 20% 를 넘으면 G1 은 `PASS` 가 아니라 `판정 불가`** 이고
  사용자에게 보인다. 측정 불가가 많은 영상이 하드 게이트를 공짜로 통과하면 안 된다
- **원본이 이미 한계면 `원본 한계` 로 기록한다.** 최대 허용 배율
  (`reframe.maxUpscale`, `§9`)까지 확대해도 G1 을 못 넘는 경우다.
  PASS 도 FAIL 도 아니고, **self-eval 로 되돌리지 않는다.**
  결과를 사용자에게 보여주고 채팅에 한 줄 붙인다 — "조금 더 가까이서 찍으면 인물이 크게 나와요".

  > 되돌리지 않는 이유: 고칠 방법이 없기 때문이다. 원본에 인물이 작게 찍혔고 확대 여력이
  > 없으면 `Composition` 을 아무리 고쳐도 G1 이 안 올라간다. 되돌리면 영원히 결과가 안 나온다.
  > 실측 사례: `NOCXAZE8XdQ` 는 사람 높이 0.31 · 608x1080 이라 `maxUpscale` 을 걸어도
  > 0.39 로 0.55 에 못 닿는다 (`docs/findings/2026-09-25-zoom-design.md §2`).

**하드 게이트가 결과를 막지 않는 세 가지**는 이렇게 갈린다:

| 상태 | 뜻 | self-eval | 사용자에게 |
|---|---|---|---|
| `판정 불가` | 측정 불가 구간이 20% 초과 | 안 돌린다 | 보여준다 + 이유 |
| `원본 한계` | 최대 배율로도 G1 미달 | 안 돌린다 | 보여준다 + 촬영 조언 |
| `FAIL` | 고칠 수 있는데 못 맞춤 | **돌린다** | 안 보여준다 |
- **G2 는 "우리가 잘랐는가" 를 잰다.** 원본에서 마스크가 위/아래 경계에 닿지 않았는데
  크롭 뒤 닿게 된 구간의 비율이다.
  세로 숏폼에서 발이 프레임 밖으로 나가는 건 정상이다 — 공개본 72프레임 중 86% 가 그렇다.
  좌우(팔 끝)는 보지 않는다
- **분모는 프레임이 아니라 면(面) 단위다.** 위와 아래를 따로 세고 나쁜 쪽을 쓴다.
  원본에서 **아래가 이미 잘린 프레임도 위는 그대로 묻는다.**

  > 프레임 단위로 빼면 안 되는 이유: 공개본은 86% 가 발이 프레임 밖이다.
  > 그 프레임을 통째로 빼면 **머리 잘림을 잴 표본이 14% 밖에 안 남는다.**
  > 머리 잘림이야말로 G2 가 잡아야 할 것이다
  > (`docs/findings/2026-09-25-g2-measurement.md §2`).

- **위·아래 분모가 둘 다 0 이면 `판정 불가`다.** 원본에서 이미 위아래가 다 잘려 있으면
  "우리가 새로 잘랐는가" 를 물을 수 없다. 통과로 적지 않는다
- **G1 과 G2 가 충돌하면 G1 을 우선한다.** 숏폼에서 피사체가 작으면 그 시점에 끝난 영상이다 (`§0-2`)
...
- 게이트 수치는 **원본 실측에서 나온다.** 근거 없이 정한 숫자를 하드 게이트로 걸지 않는다.
  ...
  - G2 도 같은 길로 갈 뻔했다. "머리·발이 프레임 밖" 으로 재면 공개본이 **86% 탈락**한다.
    세로 숏폼에서는 그게 정상이라 **정의를 "우리가 새로 잘랐는가" 로 바꿨다.**
    임계값 5% 는 그대로 두고 분모를 고친 것이다
  - 반대로 **G1 은 고치지 않았다.** 분할 마스크로 재니 공개본이 95% 로 통과한다.
    틀린 건 임계값이 아니라 정의가 없던 것이었다. **재는 법을 정하기 전에는
    임계값을 의심하지 않는다**
...
### 값은 다시 둘로 나뉜다 — 템플릿과 자막 모양

크리에이터의 자막 스타일은 **바뀐다.** 실제로 2026-08-27 에 바뀌었고, 크리에이터도
"매번 바뀔 수 있다" 고 했다. 그런데 바뀐 건 **글꼴과 기울임뿐**이었다. 글자 크기 · 보조 크기 ·
색 · 위치 규칙 · 분절은 그대로였다 (`docs/findings/2026-09-27-secondary-10.md`).

| 값 | JSON | 누가 바꾸나 |
|---|---|---|
| **템플릿** — 크기 · 위치 · 분절 · 외곽선 두께 · 리프레임 | `caption` · `secondary` · `shadow` · `reframe` | 측정. 사용자는 안 만진다 |
| **자막 모양** — 글꼴 · 굵기 · 기울임 · 색 | `look` | **사용자가 설정에서 고른다** |

- 글꼴은 **이 Mac 에 설치된 글꼴**에서 고른다 (`MadiFont.hangulFamilies()`). 앱이 글꼴을 배포하지 않으므로
  라이선스는 사용자 것이다. 크리에이터가 편집 앱에서 쓰던 글꼴을 그대로 쓸 수 있다.
  기본값(`fontFamily: null`)은 번들 Pretendard
- 글꼴을 바꿔도 **글자 높이 · 위치는 그대로다.** 폰트 크기는 고른 글꼴의 메트릭에서 역산한다.
  보조 문구도 글자 높이(`secondary.inkHeightRatio`)로 적어 본문 글꼴에 끌려가지 않는다
- 설치되지 않은 글꼴은 **조용히 대체하지 않는다.** 스타일 검증이 실패한다
- 고르는 것이지 숫자를 넣는 게 아니다 — 글꼴 목록 · 기울임 켜기/끄기 · 색 견본.
  기울임 각도는 템플릿 값(`caption.italicSlantDeg`, 실측 10°)이다
- AI 는 `look` 도 쓸 수 없다 (`§1-2`). 사람이 고르는 값이다
...
**자막 모양(글꼴 · 기울임 · 색)은 사용자가 설정에서 바로 고른다** (위 표). 아래는 **템플릿** 값
(크기 · 위치)이 바뀌었을 때 이야기다.
...
| 본문 아래끝 | **슬롯마다 다르다** (아래 표) | 무리별 3~4편 |
| 보조 문구 베이스라인 | 아래에서 **0.2023** | 높음 — 4편 중 3편 동일 |
| 외곽선 (바깥 두께) | 4~5px @720 → 6~7.5px @1080 | 중간 |
| 보조 문구 크기 | 라틴 어센더 높이 **0.01281** (24.6px @1920. 원본 실측 24px). 옛 표기 "본문 폰트의 0.4444" | 중간 — 10편 1~2px 안 |
...
`docs/findings/2026-09-25-caption-position-10.md`. 공개본 10편 × 12프레임.

| 슬롯 | 본문 아래끝 | 근거 | 그 편들의 내용 |
|---|---|---|---|
| `upperBody` | **0.2352** | 3편 (전부 0.2354) | 앉아서 말하는 상반신 |
| `fullBody` | **0.3099** | 3편 중앙값 (0.2922~0.3141) | 서 있는 전신 |
| `lowerBody` | **0.4719** | 4편 중앙값 (0.4688~0.4828) | 바닥 · 침대의 하체 클로즈업 |

**편 안에서는 거의 완벽히 고정, 편 사이에서만 갈린다.** 그래서 영상 단위로 고른다 (`§5`).

보조 문구는 본문 아래끝에서 **상대 거리**(0.0329)로 붙인다. 본문 위치가 0.235~0.483 으로
벌어지는 동안 이 간격은 0.031~0.041 로 붙어 있었다. 절대 위치로 두면 본문이 움직일 때 따로 논다.

⚠ 슬롯 3개는 각각 **3~4편 근거**라 아직 얇다. `short.v1.json` 의 `measured` 는 계속 `false` 다.
...
**1. 리프레이밍**
Vision → 피사체 트랙 → 키프레임 → CALayer 변환.

- **통과 판정은 회귀 세트의 "깨끗한" 원본으로 한다** — 단독 인물 · 단순 배경.
  `IMG_6022`(헬스장 · 기구 · 거울 · 2인 겹침)는 사람을 못 찾는 구간이 **30%** 라
  이미 정한 규칙상 `판정 불가`(20% 초과)다.
  이 영상의 통과 조건은 "안정 추적" 이 아니라 **"앱이 `판정 불가` 로 제대로 표시하는가"** 다.
  출구가 실제로 동작하는지 검증하는 회귀 사례로 쓴다 (`§8`).
- **시작 조건: `G1 · G2` 의 "사람 높이" 정의가 문서에 적혀 있을 것.**
  정의 없이 임계값부터 맞추면 또 근거 없는 숫자가 된다 (`§8`)
- 피사체는 **사람 분할 마스크**로 잡는다 (`VNGeneratePersonSegmentationRequest`).
  관절(`VNDetectHumanBodyPoseRequest`)은 머리 꼭대기가 빠지고 클로즈업에서 아예 안 잡힌다
  (10편 60프레임에서 관절 37 · 분할 60 —
  `docs/findings/2026-09-25-pose-detection-spike.md`)
- 통과: G1 · G2 · G3
- ✅ **1단계 통과 (대용 기준: 세로 10편 + 16:9 6편).**
  크리에이터 촬영본은 3단계에서 처음 들어오니 그때 G1 · G2 · G3 도 같이 돌린다.
  근거: `docs/findings/2026-09-26-vertical-substitutes.md §6`

**촬영 원본을 기다리지 않는다.** 크리에이터 공개본은 이미 9:16 크롭본이라 리프레이밍을
검증할 수 없다. 유튜브의 다른 운동·재활 **16:9 롱폼 3~5편**을 `spike/source/raw/` 에 두고
대용으로 쓴다 (커밋하지 않는다). 조건과 판정 방식은 `docs/stage-0.spec.md`.
...
- UI 문구·에러 문구는 `Madi/UI/Copy.swift` 한 파일에. 하드코딩 금지. AI 말투로.
  **`Madi/UI/**` 는 디자인 쪽이 소유한다.** 개발은 건드리지 않는다.
  개발이 필요한 문구는 **키만** 정하고(`Madi/Review/GateNotice.swift` 처럼)
  필요한 목록을 `docs/design/copy-keys.md` 에 적어 넘긴다.
