...
1차 사용자는 물리치료사 겸 크리에이터 1명(화면 표시명 "수현쌤 스튜디오", `workspaceName` 설정값). 이후 같은 도메인의 크리에이터에게 판매 가능성을 열어둔다.
...
- `Edit` — 편집 결정. 원본이 아니라 **결정 목록**이다. `cuts[]`, `subtitleStyle`, `crop`, `speed[]`
...
4. **AI 연결** — 러너 + MCP + 설정의 AI 연결 → 채팅 편집.
